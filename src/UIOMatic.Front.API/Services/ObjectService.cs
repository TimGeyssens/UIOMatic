...
namespace UIOMatic.Front.API.Services
{
    ...
    public class ObjectService : IUIOMaticObjectService
    {
        ...
        public ObjectService(IUIOMaticHelper helper,
        ...
        public IEnumerable<object> GetAll(Type type, string sortColumn = null, string sortOrder = null)
        {
            var typeInfo = GetTypeInfo(type); 
            var attri = type.GetCustomAttribute<UIOMaticAttribute>();
            var repo = Helper.GetRepository(attri, typeInfo);

            return repo.GetAll(sortColumn ?? string.Empty, sortOrder ?? string.Empty);
        }
        ...
        {
            // I don't actually think this is the best way of doing this as we are fetching all objects
        ...
        public UIOMaticTypeInfo GetTypeInfo(Type type, bool populateProperties =  false)
        {
            ...
            return (UIOMaticTypeInfo)_memCache.GetOrCreate("SQLKataObjectService_GetTypeInfo_" + type.AssemblyQualifiedName + "_" + populateProperties, entry =>
            {
                var attri = type.GetCustomAttribute<UIOMaticAttribute>();
                ...
                foreach (var prop in props)
                {
                    ...
                    if (populateProperties)
                    {
                        ...
                        if (attri2 != null)
                        {
                            ...
                            {
                                if (prop.PropertyType == typeof(bool)) view = "";
                                if (prop.PropertyType == typeof(DateTime)) view = "";
                                if (prop.PropertyType == typeof(int) || prop.PropertyType == typeof(long)) view = "";
                            }
